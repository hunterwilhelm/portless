...
```bash
portless find <name>
portless find <name> --port-only
portless find <name> --no-worktree
```

Finds an active route by name, including the current worktree prefix. Use `--port-only` to print only its assigned app port or `--no-worktree` to find the unprefixed route.
